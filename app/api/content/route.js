...
import { getTopicWordPool, getTrustedArticles, lookupDictionary } from "../../../lib/api-library";
  ...
  try {
    ...
    if (type === "vocab") {
      ...
      const excludeWords = Array.isArray(body?.excludeWords)
        ? body.excludeWords.map((item) => String(item).toLowerCase())
        : [];
      const vocab = (await readJson(`vocab/${category}.json`)) || getFallbackVocab(category);
      const baseWords = Array.isArray(vocab?.words)
        ? vocab.words.filter((word) => !excludeWords.includes(String(word.word || "").toLowerCase()))
        : [];
      const generatedFromBase = generateVocabularySet({ vocab: { words: baseWords }, category, count, seed });

      const topicHints = {
        ielts_academic: "academic writing",
        daily_convo: "daily communication",
        tech_ai: "technology ai",
        environment: "climate environment",
        social_issues: "public policy society",
        business: "business workplace"
      };

      const topicWords = await getTopicWordPool(topicHints[category] || category, count * 3);
      const seen = new Set(generatedFromBase.map((item) => String(item.front || "").toLowerCase()));
      const externalCards = [];

      for (const token of topicWords) {
        const normalized = String(token).toLowerCase();
        if (seen.has(normalized) || excludeWords.includes(normalized)) continue;
        seen.add(normalized);
        externalCards.push({
          id: `${category}-api-${normalized}`,
          front: normalized,
          back: {
            definition: "",
            phonetic: "",
        ...
        if (externalCards.length >= count) break;
      }

      const payload = [...generatedFromBase, ...externalCards].slice(0, count);

      const enriched = await Promise.all(
        payload.map(async (item, idx) => {
          if (idx > 9) return item;
