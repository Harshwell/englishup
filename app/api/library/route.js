...
import { getConversationContext, getTopicWordPool, getTrustedArticles, lookupDictionary } from "../../../lib/api-library";
...
export async function POST(req) {
  try {
    ...
    if (type === "topic_words") {
      ...
      const max = Math.min(50, Math.max(5, Number(body?.max || 30)));
      const items = await getTopicWordPool(topic, max);
      return NextResponse.json({ items });
    }
