...
async function searchWikipediaSummary(query) {
  ...
  try {
    ...
    return saveCache(key, { ok: false, source: "Wikipedia", failureReason: "api_down", latencyMs: Date.now() - startedAt }, 1000 * 60 * 5);
  }
}
...
export async function getMaterialEnrichment({ query = "education", word = "study", limit = 4, topicKey = "", cefr = "intermediate" } = {}) {
  ...
  return {
    ...
    aiUsed: false,
    fallbackReason: dictionary?.ok === false && wikipedia?.ok === false && !articles?.length ? "api_down" : undefined,
    dictionary,
    wikipedia: wikipedia?.ok === false ? null : wikipedia,
    articles,
    sources: [
      { source: "DictionaryAPI.dev", ok: dictionary?.ok !== false, latencyMs: dictionary?.latencyMs, cached: dictionary?.cached },
      { source: "Datamuse", ok: dictionary?.ok !== false, latencyMs: dictionary?.latencyMs, cached: dictionary?.cached },
      { source: "Wikipedia", ok: wikipedia?.ok !== false, latencyMs: wikipedia?.latencyMs, cached: wikipedia?.cached },
      { source: "trusted_articles", ok: Array.isArray(articles) && articles.length > 0 }
    ],
