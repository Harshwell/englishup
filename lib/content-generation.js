...
function normalizeQuestion(question, index) {
  ...
  if (!Number.isInteger(answer) && rawOptions.length) {
    const flagged = rawOptions.findIndex((opt) => opt && typeof opt === "object" && opt.is_correct === true);
...
function rotateQuestion(question, seedKey = "seed") {
  const options = Array.isArray(question?.options) ? question.options : [];
  if (!options.length) return question;

  const rand = mulberry32(hashSeed(`${seedKey}-${question.id || question.question}`));
  const offset = Math.floor(rand() * options.length);
  if (offset === 0) return question;

  const rotatedOptions = options.map((_, idx) => options[(idx + offset) % options.length]);
  const rotatedAnswer = (question.answer - offset + options.length) % options.length;

  return {
    ...question,
    options: rotatedOptions,
    answer: rotatedAnswer,
    explanation: `${question.explanation} (Variant ${offset})`
  };
}

function buildVariantSentence(word, category, seed) {
  ...
  ], `${category}-${seed}-${word.word}`);

  return `In a ${contexts[0]}, learners should use "${word.word}" precisely to improve clarity and band score.`;
}
...
function buildGrammarFromBase(base, topicId, seed) {
  ...
  const quiz = shuffle(safeBase.quiz || [], `grammar-quiz-${topicId}-${seed}`)
    .slice(0, 5)
    .map((item, idx) => rotateQuestion(normalizeQuestion(item, idx), `grammar-${topicId}-${seed}-${idx}`));
...
export function generateReadingSet({
  ...
  return picked.map((item, idx) => {
    ...
    ).slice(0, 5).map((q) => rotateQuestion(q, `reading-${seed}-${idx}`));
...
export function generateVocabularySet({
  ...
  return picked.map((word, idx) => ({
    ...
    back: {
      ...
      example: word.example || buildVariantSentence(word, category, seed),
